...
def create_timeline():
    ...
    try:
        with open("live_latency.json", "r") as f:
            data = json.load(f)
            latency_mins = data.get("latency_minutes", 65)
    except FileNotFoundError:
        latency_mins = 65

    # Determine diagram width based on how long it's been running
    # Let's scale to max 100 mins for now, but 800px width
    width = 800
    height = 400
    ...
    # X axis mapping (0 to latency_mins+10)
    ...
    events = [
        ...
        {"min": 58, "label": "10:00 False Alarm", "domain": 3},
        {"min": 60, "label": "1-Hour Milestone", "domain": 0},
        {"min": latency_mins, "label": "Current State", "domain": 0}
    ]
    ...
    for ev in events:
        ...
        label_y = y - 20 if ev["domain"] > 1 else y + 20
    ...
    # Summary box
    dwg.add(dwg.rect(insert=(20, 330), size=(760, 50), rx=5, ry=5, fill='#2a2a2a'))
    ...
    dwg.add(dwg.text(summary_text, insert=(40, 360), fill='#ffffff', font_size='14px', font_family='sans-serif'))
